...
Handling Symbolic Artifacts, Ordered vs. Unordered Trees, and Summation vs. Averaging in Our Unified Generating‐Function Framework
...
1. Negative or zero expansions at small  for TCI and cherry count, contrary to combinatorial expectations.

2. Discrepancies (sometimes factor‐of‐2, sometimes larger) for Colless or Sackin₂ when comparing with dynamic‐programming (DP) results enumerating ordered trees.
...
A.1 Symptom: Negative Coefficients at Small 
...
Letting  (for TCI) or  (for cherry) recovers the “univariate” expansions we want for total sums. However, many symbolic systems, by default, choose the principal real branch of each square root. That branch can yield negative or zero expansions at small , leading to:

• TCI(1) = \(-\tfrac12\) instead of 0,
...
In pure combinatorial terms, we want a formal power series expansion around  such that . In real analysis, the principal square root near  might produce an absolute‐value or negative sign if the expression  is evaluated for \(x\in(\tfrac12,\dots)\). Sympy or Maple thus might expand “” at small , yielding negative constants.
...
1. Interpret these expressions purely as formal power series, i.e. forcibly choose .

2. Avoid direct “numeric” evaluations near  or  that might invoke real‐analysis sign conventions.



In Section 4.4 and 4.5, we do reference potential “square‐root branch cut issues,” but we now emphasize more strongly:



Clarification: “When extracting coefficients from  or , one must expand each  in the positive/combinatorial branch around . Otherwise, small‐ terms can be negative or vanish incorrectly.”
...
B.1 Symptom: Mismatches for 



Some readers found that, for small , the Colless generating function from Section 4.3 yields:

• Colless(3) = 1 but a DP code enumerating all ordered shapes yields 2.

• Factor‐of‐2 or bigger differences at subsequent .



Similarly, Sackin₂ expansions from Section 4.6 can be smaller than the ordered sum from a standard DP. This led to confusion about “which formula is right.”
...
• Average index (divide by the number of shapes), or even

• A partial weighting.



Our main text’s Colless and Sackin₂ formulas are correct for some definition: typically, either the unrooted shape space or possibly the average over ordered shapes. But if a user’s DP is summing the Colless index over ordered shapes, the coefficient from the paper’s formula for Colless might represent half or a fraction of that total.
...
We now add a short note:



Clarification: “The Colless and Sackin₂ closed‐form expressions in Sections 4.3 and 4.6 assume [explicitly define: e.g., ‘unordered shapes’ or ‘averages’]. If one wishes to match a DP code enumerating ordered shapes and summing the entire index, a factor of 2 (or more) discrepancy may arise at small . One must scale or re‐derive the formula accordingly.”



Such a note ensures that future readers (a) confirm which shape‐space they are enumerating, (b) unify that with the formula. Indeed, a “multiplicative factor of 2” often resolves the mismatch for Colless at .
...
C.1 Symptom: Failing “S(x) = x + 2xS(x) + x^2S(x)^2”



Some implementers tried verifying a univariate identity S(x) - x - 2*x*S(x) - x^2*S(x)^2 = 0 and found it gave a nonzero expression. They concluded the Sackin generating function was “wrong.”
...
Sections (4.2) and (A.1) show how the bivariate function  satisfies
...
One must carefully substitute  and interpret each subtree factor  to get the univariate generating function for the total Sackin index. The naive “S(x)=x+2xS(x)+x^2S(x)^2” is not correct as a univariate identity unless you re‐encode the step “each leaf depth increments by 1, so multiply subtrees by .”



Hence, the “functional identity” in univariate form can differ from the naive rewriting. The main text does mention that the single‐variable version arises from letting  with the correct shift, but we recognize, in hindsight, that a more explicit example could be added.



Clarification: “Equation (4.2.1) is derived for the bivariate function . After substituting , the resulting univariate generating function S(x) does not trivially satisfy S(x)=x+2xS(x)+x^2S(x)^2; that simpler recurrence is purely bivariate. The final univariate function indeed matches the DP sums once the shift is accounted for (Section A.1).”
...
• Manually define  as a formal series, or

• If you see negative expansions, multiply by  or re‐specify the branch.

Typically, formal expansions around  yield the correct sign automatically if forced to start with “.”

2. Colless & Sackin₂: Confirm which trees (ordered? unordered?), and whether the formula yields “sum” or “average.” A short dynamic‐programming check at  or  instantly reveals if a factor‐of‐2 or bigger mismatch is present. If so, you must rescale or re-derive the final expression for your chosen interpretation.

3. Bivariate → Univariate: For Sackin (and similarly TCI, Cherry in a bivariate sense), the univariate expansions require a special substitution that might not be captured by an off‐the‐shelf identity like “.” Instead, rely on the direct polynomial expansions from the correct functional equation, or follow the step‐by‐step in Appendix A.
...
2. The discrepancies encountered by some readers or implementers reflect “symbolic branch” choices and “ordered vs. unordered / sum vs. average” differences.
